...
-- local voices = { {text = 'Venha fazer sua Addon comigo, exemplo: first citizen addon ou second citizen addon.'} }
...
-- dreamwarrior START -- 
function dreamwarriorFirst(cid, message, keywords, parameters, node) 
...
if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,dreamwarrior) 
    if addon == -1 then 
        if getPlayerItemCount(cid,34807) >= 5 then 
        if doPlayerRemoveItem(cid,34807,5) then 
            selfSay(newaddon, cid) 
              
            doSendMagicEffect(getCreaturePosition(cid), 13) 
            doPlayerAddOutfit(cid, 1146, 2) 
            doPlayerAddOutfit(cid, 1147, 2) 
            setPlayerStorageValue(cid,dreamwarrior,1) 
        end 
        else 
            selfSay(noitems, cid) 
        end 
...
function dreamwarriorSecond(cid, message, keywords, parameters, node) 
    ...
    if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,dreamwarrior+1) 
    if addon == -1 then 
        if getPlayerItemCount(cid,34806) >= 1 then 
        if doPlayerRemoveItem(cid,34806,1) then 
            selfSay(newaddon, cid) 
              
            doSendMagicEffect(getCreaturePosition(cid), 13) 
            doPlayerAddOutfit(cid, 1146, 1) 
            doPlayerAddOutfit(cid, 1147, 1) 
            setPlayerStorageValue(cid,dreamwarrior+1,1) 
        end 
        else 
            selfSay(noitems, cid) 
        end 
...
-- HAND OF THE INQUISITION START -- 
function InquisitionFirst(cid, message, keywords, parameters, node) 
    ...
    if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,inquisition) 
    if addon == -1 then 
        if getPlayerItemCount(cid,36573) > 0 then 
        if doPlayerRemoveItem(cid,36573, 1) then 
            selfSay(newaddon, cid) 
              
            doSendMagicEffect(getCreaturePosition(cid), 13) 
            doPlayerAddOutfit(cid, 1243, 1) 
            doPlayerAddOutfit(cid, 1244, 1) 
            setPlayerStorageValue(cid,inquisition,1) 
        end 
        else 
            selfSay(noitems, cid) 
        end 
...
function InquisitionSecond(cid, message, keywords, parameters, node) 
    ...
    if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,inquisition+1) 
    if addon == -1 then 
        if getPlayerItemCount(cid,36572) > 0 then 
        if doPlayerRemoveItem(cid,36572,1) then 
            selfSay(newaddon, cid) 
              
            doSendMagicEffect(getCreaturePosition(cid), 13) 
            doPlayerAddOutfit(cid, 1243, 2) 
            doPlayerAddOutfit(cid, 1244, 2) 
            setPlayerStorageValue(cid,inquisition+1,1) 
        end 
        else 
            selfSay(noitems, cid) 
        end 
...
function DruidSecond(cid, message, keywords, parameters, node) 
    ...
    if addon == -1 then 
        if getPlayerItemCount(cid,5906) >= 100 and getPlayerItemCount(cid,5942) >= 1 then 
        if doPlayerRemoveItem(cid,5906,100) and doPlayerRemoveItem(cid,5942,1) then 
            selfSay(newaddon, cid) 
...
function PirateSecond(cid, message, keywords, parameters, node) 
    ...
    if addon == -1 then 
        if getPlayerItemCount(cid,6099) >= 1 and getPlayerItemCount(cid,11425) >= 1 and getPlayerItemCount(cid,6101) >= 1 and getPlayerItemCount(cid,6102) >= 1 then 
        if doPlayerRemoveItem(cid,6099,1) and doPlayerRemoveItem(cid,11425,1) and doPlayerRemoveItem(cid,6101,1) and doPlayerRemoveItem(cid,6102,1) then 
            selfSay(newaddon, cid) 
...
keywordHandler:addKeyword({'help'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "O que voce faz eh simplesmente digitar 'first citizen addon' ou 'second citizen addon' onde voce substitui 'citizen' por qualquer addon para a qual voce gostaria de obter os addons. Supondo que voce ja coletou todas os itens necessarios, diga 'yes' e pronto, parabens voce conseguiu um addon!"}) 
keywordHandler:addKeyword({'ajuda'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "O que voce faz eh simplesmente digitar 'first citizen addon' ou 'second citizen addon' onde voce substitui 'citizen' por qualquer addon para a qual voce gostaria de obter os addons. Supondo que voce ja coletou todas os itens necessarios, diga 'yes' e pronto, parabens voce conseguiu um addon!"}) 
 ...
 node18 = keywordHandler:addKeyword({'second druid addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second druid addon you need to give me, 100 demon dusts and a blessed wooden stake. Do you have them with you?'}) 
 ...
 node24 = keywordHandler:addKeyword({'second pirate addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second pirate addon you need to give me brutus bloodbeard\'s hat, lethal lissy\'s shirt, ron the ripper\'ssabre and deadeye devious\' eye patch. Do you have them with you?'}) 
 ...
 node29 = keywordHandler:addKeyword({'second shaman addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second shaman addon you need to give me 5 voodoo doll and a mandrake. Do you have them with you?'}) 
...
-- elementalist end
-- warmaster
    node51 = keywordHandler:addKeyword({'first warmaster addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first Warmaster addon you need to give me 1 Serpent Crest. Do you have them with you?'}) 
    node51:addChildKeyword({'yes'}, WarmasterFirst, {}) 
    ...
    node52 = keywordHandler:addKeyword({'second warmaster addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second Warmaster addon you need to give me 1 Tribal Crest. Do you have them with you?'}) 
    node52:addChildKeyword({'yes'}, WarmasterSecond, {}) 
...
-- hand of the inquisition
    node53 = keywordHandler:addKeyword({'first inquisition addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first Hand of the Inquisition addon you need to give me 1 Final Judgement. Do you have them with you?'}) 
    node53:addChildKeyword({'yes'}, InquisitionFirst, {}) 
    node53:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true}) 

    node54 = keywordHandler:addKeyword({'second inquisition addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second Hand of the Inquisition addon you need to give me 1 Shadow Cowl. Do you have them with you?'}) 
    node54:addChildKeyword({'yes'}, InquisitionSecond, {}) 
    node54:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true}) 
-- hand of the inquisition


-- hand of the inquisition
    node55 = keywordHandler:addKeyword({'first hand of the inquisition addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first Hand of the Inquisition addon you need to give me 1 Final Judgement. Do you have them with you?'}) 
    node55:addChildKeyword({'yes'}, InquisitionFirst, {}) 
    node55:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true}) 

    node56 = keywordHandler:addKeyword({'second hand of the inquisition addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second Hand of the Inquisition addon you need to give me 1 Shadow Cowl. Do you have them with you?'}) 
    node56:addChildKeyword({'yes'}, InquisitionSecond, {}) 
    node56:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true}) 
-- hand of the inquisition

-- dreamwarrior
    node57 = keywordHandler:addKeyword({'first dream warrior addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first dream warrior addon you need to give me 5x Pomegranate. Do you have them with you?'}) 
    node57:addChildKeyword({'yes'}, dreamwarriorFirst, {}) 
    node57:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true}) 

    node58 = keywordHandler:addKeyword({'second dream warrior addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second dream warrior addon you need to give me 1x Ice Shield. Do you have them with you?'}) 
    node58:addChildKeyword({'yes'}, dreamwarriorSecond, {}) 
    node58:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true}) 
-- dreamwarrior


keywordHandler:addKeyword({'addons'}, StdModule.say, {npcHandler = npcHandler, text = 'Eu posso te oferecer first & second addons dos seguintes addons: Citizen, Hunter, Mage, Knight, Nobleman, Summoner, Warrior, Barbarian, Druid, Wizard, Oriental, Pirate, Assassin, Beggar, Shaman, Norseman, Elementalist, Nightmare, Jester, Brotherhood, wayfarer, pumpkin, yalaharian, glooth, rift, elementalist, warmaster, inquisition and dream warrior'})
keywordHandler:addKeyword({'duvidas'}, StdModule.say, {npcHandler = npcHandler, text = 'O que voce faz eh simplesmente digitar first citizen addon ou second citizen addon onde voce substitui citizen por qualquer addon para a qual voce gostaria de obter os addons. Supondo que voce ja coletou todas os itens necessarios, diga yes e pronto, parabens voce conseguiu um addon!'})
    
npcHandler:setMessage(MESSAGE_GREET, 'Seja bem vindo, |PLAYERNAME|. como posso ajuda lo? eu trabalho com {addons}, mas voce tem {duvidas} de como funciona??')
npcHandler:setMessage(MESSAGE_FAREWELL, 'Ate logo. recomende meus services para seus amigos.')
