...
void Combat::doTargetCombat(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		...
		if (casterPlayer) {
			...
			if (caster) {
				casterPlayer = caster->getPlayer(); // Reutiliza a variável `casterPlayer` já existente
				if (casterPlayer) {
			...
			if (target) {
				targetPlayer = target->getPlayer(); // Reutiliza a variável 'targetPlayer' já existente
				if (targetPlayer) {
