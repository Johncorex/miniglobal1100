...
do
	...
	#PS: o arquivo gdb_config deve estar na pasta do datapack
	gdb --batch -return-child-result --command=gdb_config --args ./tfs 2>&1 | awk '{ print strftime("%F %T - "), $0; fflush(); }' | tee "logs/$(date +"%F %H-%M-%S.log")"
	mysqldump -u$usersql -p$sqlpassword --add-drop-table --add-locks --allow-keywords --extended-insert --quick --compress $servername > /var/tfs/database/$databasefile
	gzip /var/tfs/database/$databasefile-f
